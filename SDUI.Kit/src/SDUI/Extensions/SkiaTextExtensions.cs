...
namespace SDUI
{
    ...
    public static class SkiaTextExtensions
    {
        ...
        /// Draws a string of text.
        /// </summary>
        public static void DrawText (this SKCanvas canvas, string text, RectangleF bounds, Control control, ContentAlignment alignment, int selectionStart = -1, int selectionEnd = -1, SKColor? selectionColor = null, int? maxLines = null, bool ellipsis = false)
            => canvas.DrawText (text, control.CurrentStyle.GetFont (), control.LogicalToDeviceUnits (control.CurrentStyle.GetFontSize ()), bounds, control.Enabled ? control.CurrentStyle.GetForegroundColor () : Theme.ForegroundDisabledColor, alignment, selectionStart, selectionEnd, selectionColor, maxLines, ellipsis);

        /// <summary>
        /// Draws a string of text.
        /// </summary>
        public static void DrawText (this SKCanvas canvas, string text, SKTypeface font, int fontSize, RectangleF bounds, SKColor color, ContentAlignment alignment, int selectionStart = -1, int selectionEnd = -1, SKColor? selectionColor = null, int? maxLines = null, bool ellipsis = false)
        {
            if (string.IsNullOrWhiteSpace (text))
                return;

            var tb = TextMeasurer.CreateTextBlock (text, font, fontSize, bounds.Size.ToSize(), TextMeasurer.GetTextAlign (alignment), color, maxLines, ellipsis);
