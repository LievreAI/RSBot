...
namespace SDUI.Renderers
{
    ...
    public class RibbonRenderer : Renderer<Ribbon>
    {
        ...
        protected virtual void RenderItemGroup (Ribbon control, RibbonTabPage tabPage, RibbonItemGroup group, PaintEventArgs e)
        {
            ...
            foreach (var item in group.Items)
                if (item is MenuSplitterItem msi)
                    RenderMenuSplitterItem (control, tabPage, group, msi, e);
                else
                    RenderItem (control, tabPage, group, item, e);

            // Right border (group Splitter)
        ...
        /// </summary>
        protected virtual void RenderMenuSplitterItem (Ribbon control, RibbonTabPage tabPage, RibbonItemGroup group, MenuSplitterItem item, PaintEventArgs e)
        {
            // Background
        ...
        /// </summary>
        protected virtual Size GetPreferredSplitterItemSize (Ribbon control, MenuSplitterItem item, Size proposedSize)
        {
            var padding = control.LogicalToDeviceUnits (6);
