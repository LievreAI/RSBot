...
namespace SDUI
{
    ...
    public static class TextMeasurer
    {
        ...
        /// <summary>
        /// Finds the next word Splitter in the specified text.
        /// </summary>
        public static int FindNextSplitter (string text, int start, bool forward)
        {
            var len = text.Length;

            if (forward) {
                for (var i = start + 1; i < len; i++) {
                    if (IsWordSplitter (text[i]))
                        return i + 1;
                }
        ...
        /// <summary>
        /// Determines if the specified character is a word Splitter.
        /// </summary>
        public static bool IsWordSplitter (char c)
        {
            switch (c) {
                case ' ':
                case '\t':
